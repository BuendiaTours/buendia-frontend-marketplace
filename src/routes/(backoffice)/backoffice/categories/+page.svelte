<script lang="ts">
	 ...
	 * Displays a filterable, sortable, paginated table of categories.
	 */
	import * as m from '$paraglide/messages';
	import type { PageProps } from './$types';
	import type { CategoriesFilters } from './schemas/filters.schema';
	...
	const filters = $derived(data.filters as CategoriesFilters);
	...
	function applyFilterPatch(patch: {
		[K in keyof CategoriesFilters]?: PatchValue<CategoriesFilters[K]>;
	}) {
		const currentParams = page.url.searchParams;
		const newParams = patchFilters(categoriesFiltersSchema, currentParams, patch);
...
<svelte:head>
	<title>{m.categories_listPageTitle()} - Backoffice</title>
</svelte:head>

<LocationBar title={m.categories_listTitle()} breadcrumbs={data.breadcrumbs} />
	...
	<table class="table-zebra table-sm table">
		<thead>
			<tr>
				<th>
					<TableSortableHeader
						title={m.categories_columnName()}
						field="name"
						currentSort={sort}
						onSortChange={(newSort) =>
							applyFilterPatch({
								sort: newSort.field as CategoriesFilters['sort'],
								order: newSort.order
							})}
					/>
				</th>
			...
			{:else}
				{#each items as item (item.id)}
					<tr>
						<td>
							<p>
								<a href={buildUrlWithFilters(CATEGORY_ROUTES.edit(item.id), page.url.searchParams)}>
									{item.name}
								</a>
							</p>
							{#if item.description}
						...
						<td>
							{#if item.status === 'ACTIVE'}
								...
								<span>{CATEGORY_STATUS_OPTIONS.find((o) => o.id === 'ACTIVE')?.name}</span>
							{:else if item.status === 'DRAFT'}
								<div aria-label="status" class="status status-lg status-neutral mr-1"></div>
								<span>{CATEGORY_STATUS_OPTIONS.find((o) => o.id === 'DRAFT')?.name}</span>
							{:else}
								<div aria-label="error" class="status status-lg status-error mr-1"></div>
								<span>{CATEGORY_STATUS_OPTIONS.find((o) => o.id === 'INACTIVE')?.name}</span>
							{/if}
						</td>
									...
									<li>
										<a
											href={buildUrlWithFilters(
												CATEGORY_ROUTES.edit(item.id),
												page.url.searchParams
											)}
										>
											{m.categories_editButton()}
										</a>
									</li>
								</ul>
							</div>

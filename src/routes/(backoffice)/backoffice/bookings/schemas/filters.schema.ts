...
export type BookingsFilters = {
	page?: number;
	pageSize?: number;
	sort?: 'CREATED_AT' | 'ACTIVITY_DATETIME' | 'UPDATED_AT';
	order?: CriteriaSortOption;
	q?: string;
	status?: BookingStatus;
};

export const bookingsFiltersSchema: FiltersSchema<BookingsFilters> = {
	fields: {
		page: {
			parse: (raw) => {
		...
		sort: createSortField(['CREATED_AT', 'ACTIVITY_DATETIME', 'UPDATED_AT'] as const),

...
export const freeToursFiltersSchema: FiltersSchema<FreeToursFilters> = {
	fields: {
		page: {
			parse: (raw) => {
		...
		status: {
			parse: (raw) => {
				if (Object.values(FreeTourStatus).includes(raw as FreeTourStatus)) {
					return raw as FreeTourStatus;
				}
				return undefined;
			},
			serialize: (value, out) => {

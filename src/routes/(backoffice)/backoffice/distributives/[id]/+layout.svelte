<script lang="ts">
	/**
	 * Layout for distributive [id] sub-routes.
	...
	import DistributiveTabNav from '../components/DistributiveTabNav.svelte';
	...
	const breadcrumbs = $derived(
		buildBreadcrumbs(page.url.pathname, {
			label: data.distributive.name || m.distributives_breadcrumbResource()
		})
	);
	...
	setContext('distributiveToast', handleToast);
</script>

<svelte:head>
	<title>{m.distributives_editPageTitle()} - Backoffice</title>
</svelte:head>

<LocationBar title={m.distributives_editPageTitle()} {breadcrumbs} />

<DistributiveTabNav distributiveId={data.distributive.id} />

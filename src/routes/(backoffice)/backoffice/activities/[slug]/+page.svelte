...
>
	<a href={`${ACTIVITY_ROUTES.list}?${page.url.searchParams.toString()}`} class="link">
		← Volver al listado
	</a>

	<div class="flex gap-2">
		<a
			href={buildUrlWithFilters(ACTIVITY_ROUTES.edit(activity.slug), page.url.searchParams)}
		...
		<form
			method="POST"
			action={buildUrlWithFilters(ACTIVITY_ROUTES.delete(activity.slug), page.url.searchParams)}
		>
			<button
				type="submit"

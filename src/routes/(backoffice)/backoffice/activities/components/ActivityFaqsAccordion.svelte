<script lang="ts">
	/**
	 * ActivityFaqsAccordion — Manages activity ↔ FAQ relationships.
	 * Uses FAQ_RELATIONSHIP_REQUEST to create/delete faq relationships
	 * with entityType=ACTIVITY and entityId=activityId.
	 */
	...
	async function handleAdd() {
		...
		try {
			...
			selectedFaqId = '';
			showToast('success', m.activities_faqsAdded());
		} catch (err) {
			console.error('Error adding FAQ:', err);
			showToast('error', m.activities_faqsError());
		} finally {
			isAdding = false;
		}
	}
	...
	async function handleRemove(faq: FaqWithRelation) {
		...
		try {
			...
			showToast('success', m.activities_faqsRemoved());
		} catch (err) {
...
<FormAccordion name="form-activity-faqs" open>
	{#snippet title()}
		<ChecklistMinimalistic class="size-6" />
		<span>{m.activities_sectionFaqs()}</span>
	{/snippet}
	{#snippet asideContent()}
		<p class="text-xs">{m.activities_sectionFaqsDescription()}</p>
	{/snippet}
	{#snippet content()}
		<div class="flex items-end gap-2 md:col-span-12">
			<div class="flex-1">
				<label class="label text-sm" for="faqSelect">
					<span>{m.activities_labelFaq()}</span>
				</label>
				<select id="faqSelect" class="select w-full" bind:value={selectedFaqId} disabled={isAdding}>
					<option value="" disabled>{m.activities_placeholderFaq()}</option>
		...
		<div class="md:col-span-12">
			{#if faqs.length === 0}
				<div class="flex flex-col items-center gap-2 py-8">
					...
					<p class="text-base-content/50 text-sm">{m.activities_faqsEmpty()}</p>
				</div>
			{:else}

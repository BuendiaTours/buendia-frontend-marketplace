<script lang="ts">
	 ...
	 * Uses direct API calls (addMeal / removeMeal).
	 */
...
<FormAccordion name="form-activity-meals" open>
	...
	{#snippet content()}
		<!-- Add meal form -->
		<div class="space-y-3 md:col-span-12">
			<div class="flex items-end gap-2">
				<div class="form-control flex-1">
					<label class="label text-sm" for="mealKind">
						<span>{m.activities_mealsKindLabel()}</span>
					</label>
					<select id="mealKind" class="select w-full" bind:value={selectedKind}>
				...
				<div class="form-control flex-1">
					<label class="label text-sm" for="mealFormat">
						<span>{m.activities_mealsFormatLabel()}</span>
					</label>
					<select id="mealFormat" class="select w-full" bind:value={selectedFormat}>

<!--
Componente ComboBox con Melt-UI
...
<div class="melt-combobox relative {wrapperClass}">
	{#if Icon}
		<Icon
			class={`absolute start-3 top-1/2 size-5 -translate-y-1/2 ${hasValue ? 'opacity-100' : 'opacity-60'}`}
		/>
	{/if}
	<input
		use:melt={$input}
		...
		<div class="flex max-h-full flex-col gap-0 overflow-y-auto px-2 py-2" tabindex="0">
			{#each filteredItems as item, index (index)}
				<li
					...
					class="relative cursor-pointer scroll-my-2 rounded-md py-2 pr-4 pl-4 data-disabled:opacity-50"
				>

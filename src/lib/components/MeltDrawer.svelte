<script lang="ts">
	...
	const sideClasses = $derived(
		mergedConfig.side === 'left'
			? 'left-0 top-0 border-r border-gray-200'
			: mergedConfig.side === 'right'
				? 'right-0 top-0 border-l border-gray-200'
				: mergedConfig.side === 'top'
					? 'left-0 top-0 right-0 border-b border-gray-200'
					: // Bottom
						'left-0 bottom-0 right-0 border-t border-gray-200'
	);
...
{#if $dialogOpen}
	<div class="melt-drawer" use:melt={$portalled}>
		...
		<div
			use:melt={$content}
			class="melt-drawer-content fixed z-50 {sideClasses} {dimensionClasses} bg-white p-6 shadow-lg focus:outline-none"
		...
		>
			<button
				use:melt={$close}
				aria-label="Cerrar"
				class="melt-drawer-close absolute top-3 right-3 cursor-pointer rounded-full p-1 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600"
			>
				<Close class="size-5" />
			</button>

			{#if title}
				<h2 use:melt={$titleEl} class="melt-drawer-title mb-4 pr-8 font-semibold text-gray-900">
					{title}
				</h2>
			{/if}

			<div class="melt-drawer-content overflow-y-auto" style="max-height: calc(100vh - 80px);">
				{@render children?.()}
			</div>
		</div>
	</div>
